...
也可以在http://www.codesourcery.com/ 上面下载,  
可以在https://launchpad.net/gcc-arm-embedded/+download  上面下载 （也不行，也是 gcc-arm-none-eabi）
*只有在https://mirrors.tuna.tsinghua.edu.cn/armbian-releases/_toolchain/ 上面才能下载到arm-linux-gnueabi的我们需要的工具链，验证过*
 *重要，arm一定要下载 gcc-arm-none-linux-eabi 系列交叉工具链，否则编译busybox会不通过*
...
不过认真按照本文的介绍，是可以不用设置成static binary的
...
*** 制作和挂载磁盘
qemu-img create -f qcow2 test.qcow2 30M
...
挂载：mount -t vfat /dev/mmcblk0 /mnt/disk1/
...
发现是由于busybox交叉编译失误导致，init进程编译出来是x86指令集
...
可能原因1： busybox采用动态链接库编译， 和rootfs中的动态库部匹配 （排除）
可能原因2： rootfs的uid不是root  （排除）
可能原因3： 使用了 hard-float EABI的工具链， 即arm-linux-gnueabi-， 应该换成 arm-linux-gnueab （原因之一）
可能原因4： qemu -M versatilepb 是armv5的核，太老了，我们需要一个armv7-a 以上的核   （原因之一）
