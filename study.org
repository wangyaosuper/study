...
echo "York boot stage in rcS 0"
/bin/mount -a
echo "York boot stage in rcS 1"
/sbin/mdev -s
echo "York boot stage in rcS 2"
ifconfig lo up
echo "York boot stage in rcS 3"
ifconfig eth0 192.168.46.36 netmask 255.255.255.0
echo "York boot stage in rcS 4"
route add default gw 192.168.46.33
echo "York boot stage in rcS 5"
...
**** 调试启动qemu中遇到关键坑
1. 看不到开机打印
 console=ttyAMA0 很重要，这样才能看到开机日志
 不然看不到在哪里出错了
2. 内存不够
 -m 256M 
否则linux4.4起不来
...
可能原因1： busybox采用动态链接库编译， 和rootfs中的动态库部匹配
可能原因2： rootfs的uid不是root
可能原因3： 使用了 hard-float EABI的工具链， 即arm-linux-gnueabi-， 应该换成 arm-linux-gnueab
可能原因4： qemu -M versatilepb 是armv5的核，太老了，我们需要一个armv7-a 以上的核
