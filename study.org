 ...
 *都不可以 gcc-arm-none-eabi  ,  gcc-arm-linux-gnueabihf 都不可以*
...
设置环境变量
SCRIPT_DIR=$(realpath .)
export LD_LIBRARY_PATH=$SCRIPT_DIR/../gcc-linaro-10.2.1-2021.04-x86_64_arm-linux-gnueabi-/lib
export PATH=$SCRIPT_DIR/../gcc-linaro-10.2.1-2021.04-x86_64_arm-linux-gnueabi-/bin:$PATH
...
make ARCH=arm CROSS_COMPILE=arm-linux-gnueabi- LOADADDR=0x60003000 uImage -j4      (会失败，不要紧)
...
在qemu启动命令中增加--sd ~/extend/qemu/busybox-1.34.0/out/test.qcow2  ， 就可以在启动的时候挂载上sdcard
...
*迭代3，挂载了qcow2格式的sdcard*
./qemu-system-arm -M vexpress-a9 -m 256M --kernel ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/zImage --dtb ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/dts/vexpress-v2p-ca9.dtb --initrd ~/extend/qemu/busybox-1.34.0/out/rootfs.img.gz --sd ~/extend/qemu/disk/disk1.qcow2 --append "root=/dev/ram rdinit=/sbin/init console=ttyAMA0" --serial stdio --net nic --net user

*迭代4，支持了telnet，并支持了login*
./qemu-system-arm -M vexpress-a9 -m 256M --kernel ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/zImage --dtb ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/dts/vexpress-v2p-ca9.dtb --initrd ~/extend/qemu/busybox-1.34.0/out/rootfs.img.gz --sd ~/extend/qemu/disk/disk1.qcow2 --append "root=/dev/ram rdinit=/sbin/init console=ttyAMA0" --serial stdio --net nic --net user,hostfwd=tcp:127.0.0.1:2323-10.0.2.33:23

*迭代5，BEST，后台运行，有telnet，但是没有启动日志*
./qemu-system-arm -M vexpress-a9 -m 256M --kernel ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/zImage --dtb ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/dts/vexpress-v2p-ca9.dtb --initrd ~/extend/qemu/busybox-1.34.0/out/rootfs.img.gz --sd ~/extend/qemu/disk/disk1.qcow2 --append "root=/dev/ram rdinit=/sbin/init console=ttyAMA0"  --net nic --net user,hostfwd=tcp:127.0.0.1:2323-10.0.2.33:23  --serial file:/tmp/serial.out --daemonize
