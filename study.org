...
本地git目录:
  1. 本地分支     # 对应svn中.svn中的代码数据，存储在.git中，不能进行代码阅读
  2. 远端分支     # 对应服务器url + 具体服务器上的分支, 对应svn中.svn中的代码数据，存储在.git中， 不能进行代码阅读
  3. 临时区       # 这个区svn没有，git新增出来，在代码提交至本地分支前先做一次缓冲， 不能进行代码阅读
  4. 工作区       # 实际读代码，写代码的地方

区别，git在本地可以管理很多歌分支，而svn只能是一个分支。
提交代码时：  工作区  --->  add至临时区  --->  commit至本地分支 (此时产生版本号) ---> push至远端服务器
更新代码时：  远端服务器 ---> fetch至远端分支 ----> merge至本地分支和工作区 

**** 基本操作
1. 创建一个git目录
git init    #创建一个空的git目录，建立好.git
git clone <url>   #从服务器上下载一个现成的git目录，并且建立好.git
2. 修改本地代码
修改本代码之前一定要先创建一个本地分支
第一步 git branch <分支名称>   #创建本地分支名称
第二步 git checkout <分支名称>  #将本地工作区切换至该分支管理上
3. 提交本地修改至本地分支
第一步  git add <修改的文件的名称，或者.也可以> #将工作区的修改提交至临时区
第二步  git commit   #将临时区的内容提交至本地分区
4. 和服务器建立关联关系
git remote <远端分支名称> <url>   #在git目录上设置一个远端分支，名字可以随便起，通常大家用origin，并将这个分支和服务器url上的git目录关联
5. 将远端分支的修改更新至工作区
git fetch <远端分支名称>  #从服务器下载远端分支对应的最新的更新数据，仅仅下载至远端分支中，并没有merge到工作区
git merge <远端分支名称>  #将远端分支的内容merge到工作区
6. 想服务器push本地分支的修改内容
git push <远端分支名称> <本地分支名称>  #将本地分支中的内容提交给远端分支
7. 回退工作区中的修改
git checkout -- <文件名>
8. 回退临时区中的内容
...
管理方法 ftp://wangyaosuper.free3v.net      wyao123
