...
跟踪分支:
git checkout --track origin/serverfix    #将当前分支和远程origin/serverfix分支关联
git checkout -b sf origin/serverfix   #将本地sf分支和远程origin/serverfix分支关联
3. 提交本地修改至本地分支
...
git merge <远端关联>/<远端分支>  #将远端关联中的一个指定分支的内容merge到本地当前分支，以及工作区
6. 想服务器push本地分支的修改内容
...
2. 在push命令上设定提交到远端哪一个分支上
   git push origin dev:master
   其中地一个dev是本地分支名，master是远端分支名
3. 创建本地分支时关联远端分支
...
4. 如果本地分支名称和远端分支名称不一致怎么办
比如 git checkout -b dev origin/test
需要看一下.git/config 目录

*** github
**** 如何使用github的Token
现在只有通过Token才能在本地通过CLI进行提交，已经不再支持命令行密码的提交方式。
在  https://github.com/settings/tokens  上面生成一个Token（一个几十个字符的字符串）

+然后在本地git仓通过如下代码创建远端分支  https://$GH_TOKEN@github.com/owner/repo.git  ， 注意不需要$+
+如创建origin远端仓库   git remote add origin https://ghp_LfGUION0fzp42t2VA@github.com/wangyaosuper/study+

*好像更应该是按照这种方式来创建origin远端仓库*
  git remote add origin https://github.com/wangyaosuper/SwiftUI-MultipeerConnectivity_Test_Chatroom.git 
  
后续提交代码就可以  git push origin master(本地分支):master(远端分支)     命令直接提交代码，不需要输入帐号名和密码
这时git会要求输入密码，讲ghp_打头的Token字符串提交就可以了 (在Ubuntu的Document，在iClould的备忘录 和 印象笔记中有记录这个字符串)
...
** kernel学习
*** 自学整理PPT
[[file:doc/Kernel/Linux内核讨论v0.2.pptx]]
...
**** 4.4 schedule()函数群学习
4.4中schedule主入口还是:schedual()  -->  __schedule(bool preempt)  in  core.c
schedule()是壳，主要实现在__schedule()
kernel的sched_class有：
- fair_sched_class
- rt_sched_class
他们是在哪里指定的，和task的关系是什么

schedule的核心队列rq（runqueue），每一个CPU各有一个rq，rq的核心操作：
...
*** 如何下载指定分支的Kernel代码
git clone https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux-stable.git --depth 1 --branch linux-5.1.y
...
** QEMU
*** 编译QEMU
**** 构建ninjia （不编译glib，可以不用）
下载： https://github.com/ninja-build/ninja
编译：  ./configure.py --bootstrap
安装：  sudo cp ./ninja /usr/bin/

**** 构建 PCRE - Perl Compatible Regular Expressions  （不编译glib，可以不用）
下载PCRE源码 @ http://www.pcre.org/
编译：
./configure --enable-utf8 --enable-unicode-properties --prefix=/home2/extend/toolchain/pcre-8.42/out/
make
make install
配置：
需要在环境变量中增加：
SCRIPT_DIR=$(realpath .)
export PCRE_CFLAGS="-I$SCRIPT_DIR/../pcre-8.42/out/include"
export PCRE_LIBS="-L$SCRIPT_DIR/../pcre-8.42/out/lib -lpcre"
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$SCRIPT_DIR/../pcre-8.42/out/lib/


**** 构建glib 2.48 on X86 Linux (for qemu 编译） （可以采取apt直接安装glib）
此处glib，不是libc， 这是gnome团队开发的lib，gdk和gtk的底座
下载路径：   https://download.gnome.org/sources/glib/2.48/
编译：
./configure --prefix=/home/wyao/extend/toolchain/glib-2.48.0/out
make -j4
make install
...
export PCRE_CFLAGS="-I$SCRIPT_DIR/../pcre-8.42/out/include"
export PCRE_LIBS="-L$SCRIPT_DIR/../pcre-8.42/out/lib -lpcre"
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$SCRIPT_DIR/../pcre-8.42/out/lib/:$SCRIPT_DIR/../glib-2.48.0/out/lib/:$SCRIPT_DIR/../pixman-0.34.0/out/lib/
export PKG_CONFIG_PATH=$PKG_CONFIG_PATH:$SCRIPT_DIR/../glib-2.48.0/out/lib/pkgconfig:$SCRIPT_DIR/../pixman-0.34.0/out/lib/pkgconfig


其中LD_LIBRARY_PATH 和 PKG_CONFIG_PATH 对于qemu的编译非常关键。
PKG_CONFIG_PATH 是pkg-config获取各个so的.pc的路径配置， 用于在系统中注册.so库，和管理.so之间的依赖关系


**** 构建pixman1
下载，编译pixman1，make install
并且在环境变量中增加
SCRIPT_DIR=$(realpath .)
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$SCRIPT_DIR/../pcre-8.42/out/lib/:$SCRIPT_DIR/../pixman-0.34.0/out/lib/
export PKG_CONFIG_PATH=$PKG_CONFIG_PATH:$SCRIPT_DIR/../pixman-0.34.0/out/lib/pkgconfig
...
./configure --prefix=/home/wyao/extend/qemu/qemu-6.0.0/out  --target-list=aarch64-softmmu,arm-softmmu --enable-virtfs --disable-kvm
make -j4
make install 


*** 设置交叉编译链
在ARM的官网下载arm和aarch64的交叉编译链 （ *不好用， 找不到 gcc-arm-none-linux-eabi, 只能找到gcc-arm-none-eabi,编译不了busybox* ）
https://developer.arm.com/tools-and-software/open-source-software/developer-tools/gnu-toolchain/gnu-rm/downloads

也可以在http://www.codesourcery.com/ 上面下载,   但是要翻墙
可以在https://launchpad.net/gcc-arm-embedded/+download  上面下载 （也不行，这上面也只有 gcc-arm-none-eabi）
*只有在https://mirrors.tuna.tsinghua.edu.cn/armbian-releases/_toolchain/ 上面才能下载到arm-linux-gnueabi的我们需要的工具链，验证过*
 *重要，arm一定要下载 gcc-arm-none-linux-eabi 系列交叉工具链，否则busybox编不过*
 *gcc-arm-none-eabi  ,  gcc-arm-linux-gnueabihf 都不可以用于qemu-arm32的buxybox编译*



gcc-arm-linux-gnueabi-10.3-2021.07-x86_64-linux.tar.bz2 
gcc-linaro-7.3.1-2018.05-x86_64_aarch64-linux-gnu.tar.xz

设置工具链环境变量，以arm的为例,解压xz包之后，工具链在bin目录下,设置环境变量
编写环境变量脚本env_prepare_rc,内容如下

SCRIPT_DIR=$(realpath .)

export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$SCRIPT_DIR/../pcre-8.42/out/lib/:$SCRIPT_DIR/../pixman-0.34.0/out/lib/:$SCRIPT_DIR/../gcc-linaro-7.4.1-2019.02-x86_64_arm-linux-gnueabi/lib

export PKG_CONFIG_PATH=$PKG_CONFIG_PATH:$SCRIPT_DIR/../pixman-0.34.0/out/lib/pkgconfig

export PATH=$SCRIPT_DIR/../gcc-linaro-7.4.1-2019.02-x86_64_arm-linux-gnueabi/bin:$PATH


交叉编译前source env_prepare_rc


*** 编译内核 for versatile开发板  （ *废* )
你能信，编译内核还要先安装curses， 当然menuconfig才会用
sudo apt-get install libncurses5-dev

make ARCH=arm versatile_defconfig
make ARCH=arm menuconfig  （可以啥都不改）
make ARCH=arm CROSS_COMPILE=arm-linux-gnueabi- -j4 all

输出结果 linux-4.4/arch/arm/boot/zImage


*** 编译内核 for vexpress开发板

make ARCH=arm vexpress_defconfig
make ARCH=arm menuconfig  （可以啥都不改）
make ARCH=arm CROSS_COMPILE=arm-linux-gnueabi- -j4 zImage

make ARCH=arm CROSS_COMPILE=arm-linux-gnueabi- LOADADDR=0x60003000 uImage -j4      (可以不做，会失败不要紧)
...
*** 制作busybox和根文件系统
**** 编译和制作busybox
在https://busybox.net/downloads/   下载最新版本
make ARCH=arm CROSS_COMPILE=arm-linux-gnueabi- defconfig

make ARCH=arm CROSS_COMPILE=arm-linux-gnueabi- menuconfig
重要，需要在menuconfig中将busybox设置为静态连接  Build Options->[*] Build BusyBox as a static binary (no shared libs);
不是说动态链接不行，动态链接的话，一定要将rootfs中的动态库配置正确，有一定难度，第一次启动建议静态链接比较容易
*不过认真按照本文的介绍，是可以不用设置成static binary的， 是完全可以share library的*
...
**** 制作rootfs
rootfs的制作脚本 base.rootfs.gen.sh 内容为：
#!/bin/bash
...
其中etc/passwd内容为：
root:l.50wPKnwHfxc:0:0:Linux User,,,:/home/root:/bin/sh

其中etc/group内容为：
root:x:0:

**** 不错的一篇文章
这一章节转自  Freedom Embedded Balau's technical blog on open hardware, free software and security， https://balau82.wordpress.com/2010/03/27/busybox-for-arm-on-qemu/    
是同行写的根目录制作方法，可以同时参考
...
*** 支持qemu的NET模式的网络通信
NET模式类似与VmWare的net模式，qemu作为防火墙，为guest提供NET映射。
guest系统的/etc/init.rd/rcS 中增加
ifconfig eth0 10.0.2.33 netmask 255.255.255.0
route add default gw 10.0.2.2

qemu启动命令中增加--net nic --net user

--net nic --net user 不加具体配置，使用的网络属性如下：
netmask 255.255.255.0
host在内网中的ipaddress, 即NAT上行网关： 10.0.2.2
dns 10.0.2.3

*如上配置完成后，guest系统就可以ping通host机器真实网卡的ip地址*
...
例
--net nic --net user,hostfwd=tcp:127.0.0.1:2323-10.0.2.33:23
将guest的23号端口，映射到host的2323端口
putty用host的2323端口登陆guest, 相当于从外网访问guest内网


*** 制作和挂载磁盘
创建磁盘镜像文件
qemu-img create -f raw test.raw 32M
格式化成vfat fs
mkfs.vfat disk1.raw

在qemu启动命令中增加--sd ~/extend/qemu/busybox-1.34.0/out/test.raw  ， 就可以在启动的时候像Flash一样挂载上磁盘镜像

在guest系统中也可以对镜像文件进行格式化
在guest系统的/dev中可以看到对应block设备
brw-rw----    1 0        0         179,   0 Aug 31 02:55 mmcblk0

在busybox中使用 fdisk 下面的n命令可以对该sdcard 进行分区
在busybox中使用 mkfs.vfat /dev/mmcblk0  ， 对该sdcard进行格式化

下次启动busybox中设置rcS脚本自动挂载：mount -t vfat /dev/mmcblk0 /mnt/disk1/

后续长期挂载使用，可以采取修改根目录  /etc/fstab文件，将sdcard挂载点增加进去


*** 支持telnet
建立设备目录 和设备文件
mkdir /etc/pts
sudo mknod rootfs/dev/pts/0 c 136 0
sudo mknod rootfs/dev/pts/1 c 136 1
sudo mknod rootfs/dev/pts/2 c 136 2
sudo mknod rootfs/dev/pts/3 c 136 3
sudo mknod rootfs/dev/pts/4 c 136 4
sudo mknod rootfs/dev/pts/5 c 136 5
sudo mknod -m 666 rootfs/dev/ptmx c 5 2
sudo mknod rootfs/dev/null c 1 3
...
putty用host的2323端口登陆guest

*** 启动qemu
( *废* )./qemu-system-arm -M versatilepb -m 256M --kernel ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/zImage --initrd ~/extend/qemu/busybox-1.34.0/out/rootfs.img.gz --append "root=/dev/ram  rdinit=/sbin/init console=ttyAMA0" --serial file:/tmp/serial.out

*以下经过验证*

*迭代0，可以在/tmp/serial.out看内核启动日志*
./qemu-system-arm -M vexpress-a9 -m 256M --kernel ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/zImage --dtb ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/dts/vexpress-v2p-ca9.dtb --initrd ~/extend/qemu/busybox-1.34.0/out/rootfs.img.gz --append "root=/dev/ram rdinit=/sbin/init console=ttyAMA0" --serial file:/tmp/serial.out


*迭代1，可以进行stdio serial 命令行交互的*
./qemu-system-arm -M vexpress-a9 -m 256M --kernel ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/zImage --dtb ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/dts/vexpress-v2p-ca9.dtb --initrd ~/extend/qemu/busybox-1.34.0/out/rootfs.img.gz --append "root=/dev/ram rdinit=/sbin/init console=ttyAMA0" --serial stdio


*迭代2，可以进行NET模式的网络通信*
./qemu-system-arm -M vexpress-a9 -m 256M --kernel ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/zImage --dtb ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/dts/vexpress-v2p-ca9.dtb --initrd ~/extend/qemu/busybox-1.34.0/out/rootfs.img.gz --append "root=/dev/ram rdinit=/sbin/init console=ttyAMA0" --serial stdio --net nic --net user

*迭代3，挂载了raw格式的vfat文件系统磁盘镜像*
./qemu-system-arm -M vexpress-a9 -m 256M --kernel ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/zImage --dtb ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/dts/vexpress-v2p-ca9.dtb --initrd ~/extend/qemu/busybox-1.34.0/out/rootfs.img.gz --sd ~/extend/qemu/disk/disk1.raw --append "root=/dev/ram rdinit=/sbin/init console=ttyAMA0" --serial stdio --net nic --net user

*迭代4，支持了telnet，并支持了login*
./qemu-system-arm -M vexpress-a9 -m 256M --kernel ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/zImage --dtb ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/dts/vexpress-v2p-ca9.dtb --initrd ~/extend/qemu/busybox-1.34.0/out/rootfs.img.gz --sd ~/extend/qemu/disk/disk1.raw --append "root=/dev/ram rdinit=/sbin/init console=ttyAMA0" --serial stdio --net nic --net user,hostfwd=tcp:127.0.0.1:2323-10.0.2.33:23

*迭代5，BEST，后台运行，有telnet，但是没有启动日志*
./qemu-system-arm -M vexpress-a9 -m 256M --kernel ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/zImage --dtb ~/extend/qemu/kernel_for_qemu_without_git/linux-4.4.281/arch/arm/boot/dts/vexpress-v2p-ca9.dtb --initrd ~/extend/qemu/busybox-1.34.0/out/rootfs.img.gz --sd ~/extend/qemu/disk/disk1.raw --append "root=/dev/ram rdinit=/sbin/init console=ttyAMA0"  --net nic --net user,hostfwd=tcp:127.0.0.1:2323-10.0.2.33:23  --serial file:/tmp/serial.out --daemonize

*迭代6, kernel 5.1*
./qemu-system-arm -M vexpress-a9 -m 256M --kernel ~/extend/qemu/stable_kernel_for_qemu/linux-stable-5.1/arch/arm/boot/zImage --dtb ~/extend/qemu/stable_kernel_for_qemu/linux-stable-5.1/arch/arm/boot/dts/vexpress-v2p-ca9.dtb --initrd ~/extend/qemu/busybox-1.34.0/out/rootfs.img.gz --sd ~/extend/qemu/disk/disk1.raw --append "root=/dev/ram rdinit=/sbin/init console=ttyAMA0"  --net nic --net user,hostfwd=tcp:127.0.0.1:2323-10.0.2.33:23  --serial file:/tmp/serial.out --daemonize

*迭代7, 可以进行lmbench 性能测试*
./qemu-system-arm -M vexpress-a9 -m 512M --kernel ~/extend/qemu/stable_kernel_for_qemu/linux-stable-5.1/arch/arm/boot/zImage --dtb ~/extend/qemu/stable_kernel_for_qemu/linux-stable-5.1/arch/arm/boot/dts/vexpress-v2p-ca9.dtb --initrd ~/extend/qemu/busybox-1.34.0/out/rootfs.img.gz --sd ~/extend/qemu/disk/disk1.raw --append "root=/dev/ram rdinit=/sbin/init console=ttyAMA0"  --net nic --net user,hostfwd=tcp:127.0.0.1:2323-10.0.2.33:23  --serial file:/tmp/serial.out --daemonize --icount shift=2
...
3. init找不到
在--append中增加rdinit
且rootfs.img中所有的文件和目录需要归属于root用户和root group

4. init启动失败panic
---[ end Kernel panic - not syncing: Requested init /sbin/init failed (error -8).
error -8 =  ENOEXEC

发现是由于busybox交叉编译时编译器选择失误导致，init进程编译出来是x86指令集
wyao@york-Ubuntu:~/extend/qemu/busybox-1.34.0/out/rootfs/bin$ arm-linux-gnueabi-objdump -a busybox 
arm-linux-gnueabi-objdump: busybox: file format not recognized
wyao@york-Ubuntu:~/extend/qemu/busybox-1.34.0/out/rootfs/bin$ objdump ./busybox  -a
...
5. init因为非法指令panic
[<c001a0ac>] (unwind_backtrace) from [<c0017554>] (show_stack+0x10/0x14)
[<c0017554>] (show_stack) from [<c030b5a0>] (panic+0x7c/0x1d8)
[<c030b5a0>] (panic) from [<c0022ce0>] (do_exit+0x828/0x8c0)
[<c0022ce0>] (do_exit) from [<c0023bb4>] (do_group_exit+0x3c/0xb8)
[<c0023bb4>] (do_group_exit) from [<c002cf14>] (get_signal+0xec/0x654)
[<c002cf14>] (get_signal) from [<c0016eec>] (do_signal+0xc0/0x404)
[<c0016eec>] (do_signal) from [<c00173f0>] (do_work_pending+0xb0/0xc0)
[<c00173f0>] (do_work_pending) from [<c00141bc>] (slow_work_pending+0xc/0x20)
---[ end Kernel panic - not syncing: Attempted to kill init! exitcode=0x00000004

可能原因1： busybox采用动态链接库编译， 和rootfs中部署的动态库eabi版本不匹配 （排除）
可能原因2： rootfs中关键文件蜀主uid不是root  （排除）
可能原因3： 使用了 hard-float EABI的工具链， 即arm-linux-gnueabi-， 应该换成 arm-linux-gnueab （实际原因之一）
可能原因4： qemu -M versatilepb 是armv5的核，太老了，编译工具链是v7版本，我们需要一个armv7-a 以上的核的CPU和开发版，调整开发版至vexpress-a9    （实际原因之一）


** 标准Linux非实时的问题
为什么标准Linux Kernel的实时性不行：
1. 临界区
2. 内核不可被抢占
3. 固定的时间片调度 （最低优先级任务也会被调度）
4. Kernel 时钟周期 10ms，（保障吞吐率，牺牲实时性）
...
*** vm
**** dalvik
Good Doc : http://www.slideshare.net/jserv/understanding-the-dalvik-virtual-machine?from_search=4
**** dalvik dexopt
1. Main Flow
...
**** art: GC
***** Launch GC by Alloc Object
Heap::AllocObjectWithAllocator()
    Heap::AllocateInternalWithGc()
        CollectGarbageInternal()
***** Launch GC when new_num_bytes_allocated >= concurrent_start_bytes_
Heap::AllocObjectWithAllocator()
    Heap::CheckConcurrentGC()
        Heap::RequestConcurrentGCAndSaveObject()
...
***** GC Process
Heap::CollectGarbageInternal()
    collector = FindCollectorByGcType(gc_type);    /// Choose a collector, by gc_type and collector_type
                                                   /// gc_type: sticky, partial, full
                                                   /// collector_type : MS, CMS, SS, GSS, MC, CC, so on
                                                   /// collector: MarkSweep, PartialMarkSweep, StickyMarkSweep
    MarkSweep::Run()
...
ImageSpace::Create()   in the art/runtime/gc/space/image_space.cc
The option item named -Ximage

VM use ImageSpace::Init() to load boot.art
Runtime::Init()
    new Heap()
        ImageSpace::Create()
            ImageSpace::Init()
    ClassLinker::InitFromImage()      init class_roots_

ClassLinker::AddBootImageClassesToClassTable()
        ...
        BaseDexClassLoader.findClass()
            DexPathList.findClass()
                DexFile.loadClassBinaryName()
                    DexFile.defineClass()
                        DexFile_defineClassNative()  // in the dalvik_system_DexFile.cc
                            DexFile::FindClassDef()  // art/runtime/dex_file.cc
                            ....   // TODO
**** dex2oat
***** change the dex file
CompilerDriver::CompileMethod()  // compiler_driver.cc    compile a method , into both arm and dex
    QuickCompiler::Compile()   // quick_compiler.cc  into arm
    DexCompiler::Compile()  // dex_to_dex_compiler.cc    into dex
        DexCompiler::CompileInvokeVirtual()   // dex_to_dex_compiler.cc
            CompilerDriver::ComputeInvokeInfo()  // get the vtable id
            inst->SetOpcode()
            inst->SetVRegB_3rc()

NOTICE: The field access will also cause the dex been changed

**** exception
Thread::QuickDeliverException()  // thread.cc
QuickExceptionHandler    // quick_exception_handler.cc
Throwable   // throwable.cc
ArtMethod::FindCatchBlock 
CatchHandlerIterator  // dex_file.h
...
**** How do ActivityThread.bindApplication() load the APK
ActivityThread.bindApplication()
    ActivityThread.handleBindApplication()
        data.info = getPackageInfoNoCheck(data.appInfo, data.compatInfo);   
        // the data.info is typed of LoadedApk, which contain the class loader for the APK
            getPackageInfo()
                new LoadedApk()
                    mClassLoader = ApplicationLoaders.getDefault().getClassLoader(zip, lib, mBaseClassLoader);
                    // zip is string valued with the path of the zip files
                    // lib is string valued with the path of the lib files
                        PathClassLoader pathClassloader = new PathClassLoader(zip, parent);  // using PathClassLoader
        
        java.lang.ClassLoader cl = instrContext.getClassLoader(); // the classloader of instrContext is equal to the one of data.info
        mInstrumentation = (Instrumentation) cl.loadClass(data.instrumentationName.getClassName()).newInstance();
        mInstrumentation.init()
        

**** Activity Resume Flow
ActivityStack.resumeTopActivityInnerLocked()
    ActivityThread.scheduleResumeActivity()
        sendMessage H.RESUME_ACTIVITY
...
*** Activity and Window
Good document
Overview  http://blog.csdn.net/luoshengyang/article/details/8462738
WMS organize win  http://blog.csdn.net/luoshengyang/article/details/8462738 
WMS AMS APP communication http://blog.csdn.net/luoshengyang/article/details/8498908

 
Activity aggregate a PhoneWindow as mWindow.
App using setContentView in onCreate() to layout the UI.

Activity.onCreate()
    Activity.setContentView(res_id)
        PhoneWindow.setContentView(res_id)
            LayoutInflater.inflate(res_id, parentContent)


ViewRootImpl.profileRendering()
ViewRootImpl.draw()   /// this is only animating, recurse to draw and layout
...
ViewRootImpl.handleAppVisibility()
ViewRootImpl.handleGetNewSurface()
ViewRootImpl.DisplayListener.onDisplayChanged()
ViewRootImpl.requestFitSystemWindows()
ViewRootImpl.requestLayout()
ViewGruop/View/ViewOverlay/ViewParent.invalidateChild()
    ViewRootImpl.invalidateChild()
        ViewRootImpl.invalidateChildInParent()
            ViewRootImpl.invalidateRectOnScreen()
ViewRootImpl.handleWindowContentChangedEvent()
    ViewRootImpl.invalidateRectOnScreen()
ViewRootImpl.setWindowStopped()
...
1. Concept
difference between AppWindowToken and WindowToken
mTokenMap
stack, task, session
WindowStat   key concept. The connect between WindowStat and ApWindowToken
...
** using tools
*** smali and baksmali
baksmali the service.odex on a preodexed Lollipop.
 2051  adb pull /system/framework/arm64/boot.oat
 2057  java oat2dex.jar boot boot.oat
 2058  java -jar oat2dex.jar boot boot.oat
 2059  cd odex
 2064  adb pull /system/framework/arm64/services.odex
 2069  java -jar oat2dex.jar services.odex .
 2073  java -jar baksmali-2.0.5.jar -a 21 -x services.dex
 2083  java -jar smali-2.0.5.jar -a 21 out -o classes.dex
 2086  zip -jq services.jar classes.dex
 2089  adb remount
 2090  adb shell rm /system/framework/services.jar
 2091  adb shell rm /system/framework/arm64/services.odex
 2092  adb shell rm -rf /data/dalvik-cache/arm*
 2093  adb shell rm -rf /data/android_logs/*
 2097  adb push services.jar /system/framework/services.jar
 2098  adb reboot


** Emulator & qemu
*** how to start emulator for Android 64bits
...
*** how to start emulator for Android 32bits
./prebuilts/android-emulator/linux-x86_64/emulator-x86 -sysdir ./out/target/product/generic_x86/ -data userdata.img -kernel ./prebuilts/qemu-kernel/x86/kernel-qemu -memory 2048 -partition-size 1000 -gpu off &
*** how to start emulator with sdcard
**** generate a sdcard image
http://developer.android.com/guide/developing/tools/mksdcard.html
mksdcard -l mySdCard 1024M sdcard.img
mksdcard is at sdk/tools/mksdcard
**** start emulator with sdcard
./prebuilts/android-emulator/linux-x86_64/emulator-x86 -sysdir ./out/target/product/generic_x86/ -data userdata.img -kernel ./prebuilts/qemu-kernel/x86/kernel-qemu -memory 2048 -partition-size 1000 -gpu off -sdcard sdcard.img
...
*** fastboot nexus5 on ubuntu 
When I use fastboot, it said no permission.
wyao@HackerToolBox:~/Downloads/m_preview/hammerhead-MPZ44Q$ fastboot devices
no permissions	fastboot

So I give the fastboot permission
wyao@HackerToolBox:~/Downloads/m_preview/hammerhead-MPZ44Q$ sudo chown root:root /home/wyao/software/adt/sdk/platform-tools/fastboot
sudo chmod +s /home/wyao/software/adt/sdk/platform-tools/fastboot

and then it will work well.
fastboot devices
077983c843a77f94	fastboot
...
* AI
** Framework
leaf   machine learning  written by Rust

expert systems
    ExSys：第一個商用專家系統。
    Mycin：一个诊断系统，其表现出人意料的好，误诊率达到专家级水平，超出一些诊所的医生。
    Siri：一個通過辨識語音作業的專家系統，由蘋果公司收購並且推廣到自家產品內作為一個人秘書功能。
http://www.infoq.com/cn/articles/zjl-siri
